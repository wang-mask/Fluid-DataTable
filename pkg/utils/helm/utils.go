...
func InstallRelease(name string, namespace string, valueFile string, chartName string, engineName string) error {
	...
	path := "/data1/wwx/Fluid-DataTable-main/charts/alluxio/"
	setMasterName := "master.name=" + engineName
	// helm 对数组类型进行传值
	// --set arr[0]=val
	// helm 使用set对map类型进行传值
	setLabelName := "worker.nodeSelector." + common.LabelAnnotationStorageCapacityPrefix + namespace + "-" + name + "=ok"
	log.V(1).Info("parse labelName")
	args := []string{"install", name, "-f", path + "values.yaml", "--namespace", namespace,
		"--set", setMasterName,
		"--set", setLabelName,
		chartName}
