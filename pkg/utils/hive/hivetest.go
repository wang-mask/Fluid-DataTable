...
func main() {
	conn, err := gohive.Connect("210.28.132.15:10000", gohive.DefaultOptions) // 无用户名、密码
	//conn, err := gohive.ConnectWithUser("210.28.132.15:10000", "username", "password", gohive.DefaultOptions) // 需要用户名、密码
	if err != nil {
	...
	// 	fmt.Println(c1)
	// }
	println(TableLocationNotChange(conn, "reason"))
	conn.Close()
}
